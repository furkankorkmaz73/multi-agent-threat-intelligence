...
class RecommenderAgent:
    def suggest(
        ...
        critic_review = analysis_result.get("critic_review", {}) or {}
        ...
        if related_urlhaus_count > 0:
            recommendations.append("Add related IOC values to firewall, DNS, proxy or EDR block/watch lists after validation.")
        if related_dread_count > 0:
            recommendations.append("Perform analyst review because cross-source discussion suggests elevated attacker interest.")
        if "exploit_sale" in dread_categories or "access_sale" in dread_categories:
            recommendations.append("Treat matching assets as potentially targeted and validate external exposure immediately.")
        if source == "urlhaus":
            recommendations.append("Search historical network logs and DNS/proxy telemetry for this IOC.")
        if source == "cve":
            recommendations.append("Confirm whether vulnerable product/version is present in the environment.")
        if "cobaltstrike" in tags or "botnet" in tags:
            recommendations.append("Prioritize endpoint telemetry review for beaconing, staging or command-and-control patterns.")
        if structural_strength >= 0.35:
            recommendations.append("Use graph context to pivot into adjacent entities because the evidence neighborhood is structurally strong.")
        for warning in critic_review.get("warnings", []):
            recommendations.append(f"Analyst note: {warning}")
