...
This writes experimental proxy-label, baseline, optional model, leakage-check, and narrative artifacts under `agent-python/reports/thesis/`. Proxy labels are not ground truth, production `risk_score` is unchanged, evidence gates are unchanged, Dread live crawling is not used, and confidence remains separate from risk. These artifacts are diagnostic thesis material, not proof of real-world exploit prediction.
