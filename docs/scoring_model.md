...
`risk_explanation_traces.json` and `risk_explanation_traces.md` are generated thesis artifacts. They are built from structured scenario/evaluation data and link normalized signals, weights, weighted contributions, generic CVE risk, confidence, evidence-gate outcomes, and asset-aware operational-risk examples. They are explainability artifacts only and do not alter the scoring formula.
